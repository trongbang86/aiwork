...
export function buildApp(options: { database?: string; uploadRoot?: string; https?: ServerOptions } = {}) {
  ...
  app.get('/v1/projects',{schema:{tags:['Projects'],summary:'List projects'}},async()=>sqlite.prepare(`SELECT w.id,w.key,w.title,w.description,w.ai_instructions AS aiInstructions,w.version,(SELECT count(*) FROM work_items x WHERE x.project_id=w.id) AS itemCount FROM work_items w JOIN work_item_types t ON t.id=w.type_id WHERE t.key='project' ORDER BY w.created_at`).all());
